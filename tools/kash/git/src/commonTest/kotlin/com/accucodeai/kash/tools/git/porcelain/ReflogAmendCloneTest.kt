...
class ReflogAmendCloneTest {
    ...
    private suspend fun run(
        ...
        val res = cmd.run(args.toList(), ctx)
        return Output(res.exitCode, out.readString(), err.readString())
    }

    private suspend fun setup(fs: InMemoryFs) {
        fs.mkdirs("/r")
    ...
    private suspend fun commit(
        ...
        fs.writeBytes("/r/$path", bytes.encodeToByteArray())
    ...
    @Test fun stashWritesReflog() =
        runTest {
            ...
            setup(fs)
            commit(fs, "a.txt", "1\n", "one")
            fs.writeBytes("/r/a.txt", "modified\n".encodeToByteArray())
    ...
    @Test fun cloneViaRefResolverMaterializesWorkTree() =
        runTest {
            ...
            val r =
                run(fs, "/parent", adapter = adapter, args = arrayOf("clone", "https://example.com/repo.git", "repo"))
            assertEquals(0, r.rc, r.stderr)
            // Work tree should contain hello.txt.
            assertTrue(
                fs.exists("/parent/repo/hello.txt"),
                "expected hello.txt materialized:\n${r.stdout}\n${r.stderr}",
            )
            ...
            val cfg = fs.readBytes("/parent/repo/.git/config").decodeToString()
