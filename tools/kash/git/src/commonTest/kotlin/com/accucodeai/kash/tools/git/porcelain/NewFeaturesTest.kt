...
class NewFeaturesTest {
    ...
    private fun setupRepoWithCommits(fs: InMemoryFs): String {
        fs.mkdirs("/r")
        run(fs, "/r", args = arrayOf("init"))
        runBlocking { fs.writeBytes("/r/a.txt", "one\n".encodeToByteArray()) }
    ...
    @Test fun headParentAndTildeResolve() {
        ...
        val log = run(fs, "/r", args = arrayOf("log", "--oneline", "HEAD~1")).stdout.lines().filter { it.isNotBlank() }
